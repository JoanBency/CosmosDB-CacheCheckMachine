...
var numOfRecords = dataIngestion[1].numberOfRows*dataSelection[3].numOfIterations;
...
async function IngestionAndSelection(connectionSettings, connectionMode, partitionMode, callback) {
    var roundtripStartTime = performance.now();

    selectMetrics = { avgRoundtripTime: 0.0,minSelectRoundtripTime: 100000.0,maxSelectRoundtripTime: 0.0,totalRoundtripTime: 0.0,avgRequestDuration: 0.0,minRequestDuration: 100000.0,maxRequestDuration: 0.0,totalRequestDuration: 0.0,avgRequestCharge: 0.0,minRequestUnit: 100000.0,maxRequestUnit: 0.0,totalRequestUnit: 0.0,cacheHitCount: 0,pragmaNoCache: 0, pragmaElse: 0, cacheControlElse: 0, numberOfReadRegions: 0, avgThrottleRetryCount: 0, avgThrottleRetryWaitTime: 0.0,totalRecordFetched: 0,totalRecordFetchedWithCache: 0, totalRecordInserted: 0};
    ...
    try {
        ...
        console.log("\nNow we are gonna start entering data...");
        if (partitionMode == "Single Partition") {
            for(i = 1; i <= dataIngestion[1].numberOfRows; i++) {
                var newRecord = getNewRecord(partitionMode, i);

                try {
                    var { resource: insertedRecord } = await container.items.create(newRecord);
                    selectMetrics.totalRecordInserted++;

                    for(j = 0; j < dataSelection[3].numOfIterations; j++) {
                        var selectRoundtripStartTime = performance.now();
    
                        try {
                            const { resource: selectedRecord, headers: headerInformation } = await container.item(String(dataIngestion[0].productIDStart + i),0).read();
            ...
            for(i = 1; i <= dataIngestion[1].numberOfRows; i++) {
                ...
                try {
                    ...
                    for(j = 0; j < dataSelection[3].numOfIterations; j++) {
                        ...
                        try {
                            const { resource: selectedRecord, headers: headerInformation } = await container.item(String(dataIngestion[0].productIDStart + i),((dataIngestion[0].productIDStart + i)%10)).read();
...
function getNewRecord(partitionMode, i) {
    ...
    if ( partitionMode == "Single Partition" ) { 
        ...
        partitionID = (dataIngestion[0].productIDStart + i) % 10;  
    }
    else { partitionID = null; }

    var newProduct = {
        id: String(dataIngestion[0].productIDStart + i),
        ProductId: dataIngestion[0].productIDStart + i,
        ProductName: `Electronics${dataIngestion[0].productIDStart + i}`,
    ...
    // newProduct = Object.assign({id: String(dataJson[0].productIDStart + i)}, newProduct);
...
function logSelectProcessDesc(connectionSettings, connectionMode, failedQueries, partitionMode) {
    ...
    var scenarioType = `ING_PSEL2-DGW_${connectionMode == "DedicatedGateway" ? "Yes" : connectionMode == "DirectConnection" ? "No" : null}-${partitionMode == "Single Partition" ? "SP" : partitionMode == "Multiple Partition" ? "MP" : null}-PK_${partitionKeyGiven}-Run_Num#${dataSelection[2].Run_Num}`;
    console.log("\n==============================================");
    console.log("\nProcess Desc: \tIngestion and Point Selecting(V2) Data Records");
    ...
    logger.info("Process Desc: \tIngestion and Point Selecting(V2) Data Records");
   ...
   metaDataLog = 'Ingestion and Point Selecting(V2) Data Records,' + consistencyLevel + ',' + scenarioType + ',' + dedicatedGatewayUsed + ',' + connectionSettings[0].endpoint + ',' + connectionSettings[2].databaseId + ','+ connectionSettings[3].containerId + ',' + partitionMode + '-' + partitionKeyGivenOrNot + ',' + connectionSettings[4].partitionKey.paths[0] + ',' + String(numOfRecords-failedQueries) + ',' + String(failedQueries) + ',' + String(numOfRecords) + ',';
}
...
function logSelectResponseStats() {
    ...
    console.log(`\nTotal Records inserted: \t${selectMetrics.totalRecordInserted}`);
    console.log(`\nTotal Records fetched immediately: \t${selectMetrics.totalRecordFetched}`);
    ...
    logger.info(`---------------------`);
    logger.info(`Total Records inserted: \t${selectMetrics.totalRecordInserted}`);
    logger.info(`Total Records fetched: \t${selectMetrics.totalRecordFetched}`);
